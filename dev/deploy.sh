...
SERVICES="db rabbitmq backend worker frontend nginx"
...
start() {
    ...
    if [ ${#services[@]} -eq 0 ]; then
        echo -e "${GREEN}Starting all services (dev)...${NC}"
        docker compose up --build -d
    else
        echo -e "${GREEN}Starting ${services[*]} (dev)...${NC}"
    ...
    echo "  API Docs:     http://localhost/health"
...
stop() {
    local services=("${@}")
    if [ ${#services[@]} -eq 0 ]; then
        echo -e "${YELLOW}Stopping all services (dev)...${NC}"
        docker compose down
    else
